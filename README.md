...
| 文件 | 说明 |
|------|------|
| `start_ego.sh` | **一键启动脚本**（宿主机直接运行） |
| `stage_2/ego/mediapipe_ego.py` | 宿主机端：RealSense + MediaPipe + UDP |
| `stage_2/ego/simulation/mujoco_ego_sim.py` | Docker 端：仿真器主程序（单文件） |
| `stage_2/ego/simulation/mujoco_ik.py` | MuJoCo Jacobian IK 求解器（零空间正则化） |
| `stage_2/ego/simulation/xarm_mesh.xml` | xArm6 STL 网格 MJCF 模型 |
| `stage_2/simulation/xarm6.xml` | 简化 DH MJCF 模型（备用） |
| `stage_2/ego/scripts/restart_ego_sim.sh` | 仿真器重启脚本 |
...
| 仿真 | `ego/simulation/mujoco_ego_sim.py` | **EGO 遥操作仿真器**（单文件，零空间 IK） |
| 仿真 | `ego/simulation/mujoco_ik.py` | MuJoCo Jacobian IK + 零空间正则化 |
| 仿真 | `simulation/xarm6.xml` | 精确 DH MJCF 模型（FK 误差 0mm） |
| 仿真 | `ego/simulation/xarm_mesh.xml` | STL 网格 xArm6 模型（EGO 默认） |
