#!/usr/bin/env python3
"""
Train MARKER-based EGO policy: state + ego_image → action.

The goal marker position is ONLY visible through the ego camera.
The model must learn to extract 3D spatial information from 2D images.
No goal_position or marker_position is provided as input.

This is the true EGO scenario: visual servoing toward a visible target.
"""
...
SRC = "/workspace/umi/data/marker_feat_dataset"
V3 = SRC + "_v3"
if os.path.exists(V3):
    shutil.rmtree(V3)
stats = convert_directory(SRC, V3)
write_info_json(V3, stats)
write_episodes_metadata(V3, stats.episodes)
dd = os.path.join(V3, "data", "chunk-000")
dfs = [pd.read_parquet(os.path.join(dd, f))
       for f in sorted(os.listdir(dd)) if f.endswith(".parquet")]
compute_and_write_stats(V3, dfs)
print(f"v3: {V3}")

# Step 2: Dataset — image features only, NO goal or marker position
ds = GoalConditionedDataset(V3, chunk_size=100)
ds._has_goal = False
ds._has_image = True
print(f"Marker EGO Dataset: {len(ds.episodes)} eps, goal=OFF, img=ON")
...
it = iter(loader)
OUT = "/workspace/umi/outputs/act_marker"
    ...
    if (step + 1) % 2000 == 0:
        ...
        if al < best_loss:
            ...
            torch.save({"step": step + 1, "model_state_dict": model.state_dict(),
                         "cfg": cfg}, os.path.join(OUT, "best.pt"))
            print(f"    -> Best ({al:.4f})")
