...
项目当前处于**第一阶段完成、第二阶段准备中**：遥操作回路已闭环（36/36 测试通过），仿真环境已集成（MuJoCo），数据管道工具（质量过滤、LeRobot v3.0 转换、任务定义、时间同步）已就绪。
...
- **MuJoCo 仿真**：`MujocoRobotInterface`（`stage_2/simulation/mujoco_interface.py`）实现 `RobotInterface` ABC，将 MuJoCo 物理仿真集成到 safety_guardian 控制回路中。safety_node 设置 `robot_mode="mujoco"` 即可切换到仿真模式。MJCF 模型文件位于 `stage_2/simulation/xarm6.xml`，由 `generate_mjcf.py` 生成。MuJoCo 已添加到 Docker 镜像中（`pip install mujoco`），无需每次手动安装。
...
- `lerobot_v3_converter.py` — LeRobot v3.0 Parquet 格式转换器（独立使用或通过 `convert_to_lerobot --v3` 调用）
...
- `simulation/` — MuJoCo 仿真环境：MJCF 模型生成、`MujocoRobotInterface`（RobotInterface ABC 实现）、仿真管道启动文件、端到端测试
